...
This baremetal project is meant as an example show how baremetal application 
run. 
...
hello.c
  ...
  This file is the main glue for all the components.
...
1. DS must be launched from the SoC EDS Command Shell in order to have all
   ...
   in Windows by selecting Quartus <version> -> SoC Embedded Design Suite ->
...
1. In DS, build the application:
  ...
  1b. In the "Project Explorer" panel, right-mouse-click 
      "Altera-SoCFPGA-HelloWorld-Baremetal-GNU" and select "Build Project".
...
3. In DS, configure the launch configuration.
  ...
  3b. In the options on the left, expand "DS Debugger" and select
      "Altera-SoCFPGA-HelloWorld-Baremetal-GNU-Debug".
...
1. In DS, launch the debug configuration.
  ...
  1b. In the "Debug Control" panel, right-mouse-click
      ...
      "Connect to Target".
2. Configure stack and heap by running this command "source semihost_cvav_setup.ds"
    2a. Use semihost_cvav_setup.ds if you are using cyclone v or arria v board
    2b. Use semihost_arria10_setup.ds if you are using arria10 board
3. Load the examples
  3a. Run "load hello.axf"
  3b. Click debug from entrypoint
  3c. Run command "run" in the Command tab to run the application
